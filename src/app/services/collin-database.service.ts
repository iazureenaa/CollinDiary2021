...
export class CollinDatabaseService {
  ...
  async initDb() {
    return this.sqlite
      .create({ name: 'collinDB.db', location: 'default' })
      .then(
        ...
        async (collinDBHandler: SQLiteObject) => {
          ...
          await collinDBHandler
            ...
            .catch((e) =>
              console.log(
                `ref_piece_type table could not be created. Encountered error: ${e}`
              )
            );
          ...
          await this.insertRefPieceType()
            ...
            .catch((e) =>
              console.log(
                `ref_piece_type table could not be populated. Encountered error: ${e}`
              )
            );
        }
      )
  ...
  async getAllRecords(): Promise<EntryDiaryInterface[]> {
    ...
    let result = await this.database
      .executeSql('select * from entry_diary', [])
      .catch((e) => {
        console.log(e);
      });
  ...
  async insert(tableName, records) {
    ...
    switch (tableName) {
      case 'entry_diary':
        sqlText =
          'insert into entry_diary(date, no_of_piece, ref_piece_type_id) VALUES(?, ?, ?)';
        values = [records.date, records.no_of_piece, records.ref_piece_type_id];
        break;

      case 'ref_piece_type':
        sqlText =
          'insert into ref_piece_type(name, cent_value, image_url, modified_dt, type) VALUES(?, ?, ?, ?, ?)';
        values = [
          records.name,
          records.cent_value,
          records.image_url,
